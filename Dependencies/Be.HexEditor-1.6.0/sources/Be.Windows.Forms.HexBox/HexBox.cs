	...
	public class HexBox : Control
	{
		...
		class KeyInterpreter : IKeyInterpreter
		{
			...
			void UpdateMouseSelection(object sender, MouseEventArgs e)
			{
				...
				{
					realselStart = selEnd + _bpi.CharacterPosition;
					realselLength = _bpiStart.Index - selEnd + (_bpi.CharacterPosition ^ 1);
				}
				else if (selEnd > _bpiStart.Index)
				{
					realselStart = _bpiStart.Index;
					realselLength = selEnd - realselStart + _bpi.CharacterPosition;
				}
				else
				{
					realselStart = _bpiStart.Index;
		...
		BytePositionInfo GetHexBytePositionInfo(Point p)
		{
            ...
            if (p.X > _recHex.X + _recHex.Width - 10)
                p.X = _recHex.X + _recHex.Width - 10;
            /*if (p.Y > _recHex.Y + _recHex.Height)
                p.Y = _recHex.Y + _recHex.Height;*/
		...
		public void Paste()
		{
            ...
            long length = 0;
            if (_selectionLength > 0 && ((_byteProvider.SupportsInsertBytes() && buffer.Length > _selectionLength) || (_byteProvider.SupportsDeleteBytes() && buffer.Length < _selectionLength)))
                length = _selectionLength;
            else
                length = (int)Math.Min((int)_byteProvider.Length - _bytePos, buffer.Length);
            _byteProvider.DeleteBytes(_bytePos, length);

            if (buffer.Length > length)
            {
                byte[] newBuffer = new byte[(int)length];
                Buffer.BlockCopy(buffer, 0, newBuffer, 0, (int)length);
                buffer = newBuffer;
            }
        ...
        /*public bool CanPaste()
        {
            ...
            if(buffer != null)
                if (buffer.Length == _selectionLength || (_byteProvider.SupportsInsertBytes() && buffer.Length > _selectionLength) || (_byteProvider.SupportsDeleteBytes() && buffer.Length < _selectionLength))
                    return true;
            return false;
