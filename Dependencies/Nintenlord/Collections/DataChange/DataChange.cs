...
namespace Nintenlord.Collections
{
    ...
    public sealed class DataChange<T> : IDataChange<T>
    {
        ...
        public void AddChangedData(int offset, T[] data, int index, int count)
        {
            ...
            List<int> intersectedKeys = GetIntersectingKeys(offset, count);
        ...
        /// <returns></returns>
        private List<int> GetIntersectingKeys(int offset, int count)
        {
            List<int> intersectedKeys = new List<int>();
            foreach (var item in (IEnumerable<KeyValuePair<int, T[]>>)this)
            {
                if (Intersects(offset, count, item.Key, item.Value.Length))
                {
                    intersectedKeys.Add(item.Key);
                }
            }
        ...
        public int AmountOfChanges
        {
            ...
            {
                int result = 0;
                foreach (var item in dataToChange.Values)
                {
                    result += item.Length;
                }
                return result;
        ...
        public bool Equals(T[] other)
        {
            ...
            foreach (var item in dataToChange)
            {
                ...
                {
                    if (!comp.Equals(other[offset + i], item.Value[i]))
                        return false;
                }
            }
            return true;
