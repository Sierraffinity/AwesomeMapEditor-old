...
{
    static public class ReversePolishNotation
    {
        ...
        static public List<T> ShuntingYardAlgorithm<T>(
            ...
            IEqualityComparer<T> eq)
        {
            ...
            foreach (var token in tokens)
            {
                ...
                else if (operators.TryGetValue(token, out tempInfo))
                {
                    while (operatorStack.Count > 0)
                    {
        ...
        static public T Evaluate<T>(IEnumerable<T> rpnValue, IDictionary<T, OperatorInfo<T>> operators)
        {
