...
//	((unsigned char*)ROMptr)[addr / 2]
disassmebler::disassmebler(const char *filePath)
{
	string *fileName = new string(filePath);
	long fileSize = FileSize64(fileName->c_str());
	fstream file;
...
char *disassmebler::elfGetAddressSymbol(u32 addr)
{
	return NULL;
}

unsigned char disassmebler::debuggerReadByte(u32 addr)
{
	return ((char*)ROMptr)[addr];
}

unsigned short disassmebler::debuggerReadHalfWord(u32 addr)
{
	return ((short*)ROMptr)[addr/2];
}

unsigned int disassmebler::debuggerReadMemory(u32 addr)
{
	return ((int*)ROMptr)[addr/4];
}
...
char* disassmebler::addStr(char *dest, const char *src){
  while (*src){
    *dest++ = *src++;
  }
  return dest;
...
char* disassmebler::addHex(char *dest, int siz, u32 val){
  if (siz==0){
    siz = 28;
...
int disassmebler::disArm(u32 offset, char *dest, int flags){
...
int disassmebler::disThumb(u32 offset, char *dest, int flags){
