...
namespace Fire_Emblem_Map_Editor 
{
    class LZ77
    {
        ...
        public byte[] Compress(BinaryReader br, int offset, int size)
        {
            ...
            {
                ReadAheadBuffer.Enqueue(binaryReader.ReadByte());
            } 
            #endregion

            while ((binaryReader.BaseStream.Position < (offset + size)) && (binaryReader.BaseStream.Position < binaryReader.BaseStream.Length))
            {
                ...
                for (int i = 0; i < BlockSize; i++)
                {
                    if (binaryReader.BaseStream.Position < binaryReader.BaseStream.Length)
                    {
                        ...
                        if (compressedSeed[1] > 2)
                        {
                            ...
                            data[0] += (byte)(((compressedSeed[0] - 1) >> 8) & 0xF); //Check if errors.
        ...
        private int[] Search(List<byte> SlidingWindow, byte[] ReadAheadBuffer)
        {
            ...
            Offsets[0] = (SlidingWindow.Count - Offsets[0]);
            return new int[2] { Offsets[0], size };
        }
